...
in
{
  xdg.configFile = {
    ...
    # Generated by Nix so Lua configs can consume store paths without guessing
    # where Home Manager installed helper tools or Python environments.
  ...
  programs.neovim = {
    ...
    plugins =
      let
        # Home Manager loads non-optional plugin configs eagerly. Marking a
        # plugin optional keeps it installed but lets lz.n lazy-load it from Lua.
        plug = name: config: {
          plugin = name;
          type = "lua";
          config =
